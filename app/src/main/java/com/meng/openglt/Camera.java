...
public class Camera {
	...
	private final BaseBoundingBox baseBoundingBox = new BaseBoundingBox("scene",-20,20,-20,20,-20,20);
	...
	private boolean isOutOfBounds(float[] buffer) {
		if (baseBoundingBox.outOfBound(buffer[0] / buffer[3],buffer[1] / buffer[3],buffer[2] / buffer[3])){
			Log.i("Camera", "Out of scene bounds");
			return true;
		}
		/*List<Object3DData> objects = scene.getObjects();
		for (int i = 0; objects != null && i < objects.size(); i++) {
			BaseBoundingBox baseBoundingBox = objects.get(i).getBaseBoundingBox();
			// Log.d("Camera","BaseBoundingBox? "+baseBoundingBox);
			if (baseBoundingBox != null && baseBoundingBox.insideBounds(
					buffer[0] / buffer[3]
					, buffer[1] / buffer[3]
					, buffer[2] / buffer[3] )) {
